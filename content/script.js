...
// edit color
function editColor() {
    // three inputs for editing color
    const editColorIdInput = document.getElementById('edit_color_select'); 
    ...
    // parse and check input values
    const colorId = editColorIdInput.value;
    const newColorName = newColorNameInput.value;
    const newHexValue = newHexValueInput.value;

    // Check for hex code validity
    let pound = newHexValue.substring(0, 1);
    if (newColorName === '' || newHexValue === '' || !(pound === '#') || newHexValue.length !== 7) {
        document.getElementById('edit_response').innerHTML = "Incorrect value/s entered.";
    } else {

        // change your EID
        const editColorUrl = "https://cs.colostate.edu:4444/~cwagner4/TeamOne/content/database_connection.php?edit_color=1&edit_color_id=" + colorId + "&new_color_name=" + newColorName + "&new_hex_value=%23" + newHexValue.substring(1,7);
        fetch(editColorUrl)
        .then(response => {
            if (!response.ok) {
                throw new Error("Connection failed");
